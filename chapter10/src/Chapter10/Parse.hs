...
data ParseState = ParseState
    { string :: L.ByteString -- ^The current residual string.
    , offset :: Int64        -- ^The offset into the original string since parsed.
    } deriving Show
...
parseWhileVerbose p = peekByte ==> \mc ->
                      case mc of Nothing -> identity []
                                 Just c | p c ->
                                            parseByte ==> \b ->
