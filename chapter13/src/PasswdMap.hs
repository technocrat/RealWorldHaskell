...
data PasswdEntry = PasswdEntry { userName :: String
                               , password :: String
                               , uid :: Integer
                               , gid :: Integer
                               , gecos :: String
...
instance Read PasswdEntry where
    readsPrec _ value =
        ...
        where -- Takes a delimeter a list. Break up the list based on the delimeter.
          split :: Eq a => a -> [a] -> [[a]]
...
inputToMaps inp = (uidmap, usermap)
    where -- fromList converts a [(key, value)] list into a Map
...
mainMenu :: (Ord a, Read a, Show b, Show a1)
         => (Map.Map a b, Map.Map String a1)
         -> IO ()
