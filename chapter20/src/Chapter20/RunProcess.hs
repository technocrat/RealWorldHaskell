...
module Chapter20.RunProcess where
...
import           System.Posix.IO         (closeFd, createPipe, dupTo,
                                          fdToHandle, stdInput, stdOutput)
...
class CommandLike a where
    {- | Given the command and a string representing input, invokes the
       command. Returns a String representing the output of the command. -}
...
instance CommandLike SysCommand where
    invoke (cmd, args) closefds input =
        do -- Create two pipes: one to handle stdin and the other to
           -- handle stdout. We do not redirect stderr in this program.
           (stdinread, stdinwrite) <- createPipe
           (stdoutread, stdoutwrite) <- createPipe

           -- We add the parent FDs to this list because we always need to
           -- close them in the clients.
           addCloseFDs closefds [stdinwrite, stdoutread]

           -- Now, grab the closed FDs list and fork the child.
           childPID <- withMVar closefds (\fds ->
                                          forkProcess
                                          (child fds stdinread stdoutwrite))
           ...
           _ <- forkIO $ do hPutStr stdinhdl input
                            hClose stdinhdl
        ...
        where child closefds' stdinread stdoutwrite =
                  do -- Copy our pipes over the regular stdin/stdout FDs
                     ...
                     -- Clse all the open FDs we inherited from the parent
                     mapM_ (\fd ->
                            catch (closeFd fd) (\(SomeException _) ->
                                                return ())) closefds'
...
instance CommandLike String where
    invoke cmd closefds input =
        do -- Use the shell given by the environment variable SHELL,
           ...
           esh <- getEnv "SHELL"
           let sh = case esh of Nothing -> "/bin/sh"
                                Just x  -> x
...
removeCloseFDs closefds removethem =
    ...
    where procfdlist = foldl removefd

          -- We want to remove only the first occurrence at any given fd
          removefd [] _                  = []
          removefd (x:xs) fd | fd == x   = xs
                             | otherwise = x : removefd xs fd

-- Supprt for running Haskell commands
instance CommandLike (String -> IO String) where
    invoke func _ input =
        return $ CommandResult (func input) (return (Exited ExitSuccess))

-- Support pure Hasekll functions by wrapping them in IO
instance CommandLike (String -> String) where
    invoke func = invoke iofunc
        where iofunc :: String -> IO String
              iofunc = return . func

-- It's also useful to operate on lines. Define support for line-based functions
-- both within and without the IO monad.
...
{- | Type representing a pipe. A 'PipeCommand' consists of a source and
...
{- | Given two 'CommandResult' items, evaluate the exit codes for both and
   ...
   exited successfully. Otherwise, it will reflect the first error encounterd.
 -}
getEC :: CommandResult -> CommandResult -> IO ProcessStatus
getEC src dest = do
  sec <- getExitStatus src
  dec <- getExitStatus dest
  case sec of Exited ExitSuccess -> return dec
              x                  -> return x
...
* IO String runs, throws an exception on error, reads stdout into a buffer,
  and returns it as a string.

* IO [String] is same as IO String, but returns the results as lines.

* IO ProcessStatus runs and returns a ProcessStatus with the exit information.
  stdout is sent to stdout.  Exceptions are not thrown.

* IO (String, ProcessStatus) is like IO ProcessStatus, but also includes a
  description of the last command in the pipe to have an error (or the last
  command, if there was no error)

* IO Int returns the exit code from a program directly.  If a signal caused the
  command to be reaped, returns 128 + SIGNUM.

* IO Bool returns True if the program exited normally (exit code 0, not stopped
  by a signal) and False otherwise.
 -}
...
instance RunResult (IO Int) where
    run cmd = do
      rc <- run cmd
      case rc of Exited ExitSuccess     -> return 0
...
checkResult ps = case ps of Exited ExitSuccess -> return ()
                            x                  -> fail (show x)

{- | A convenience function.  Refers only to the version of 'run'that returns
   @IO ()@. This prevents you from having to cast to it all the time when you
   do not care about the result of 'run'.
 -}
...
grep pat = filter (ismatch regex)
    where regex = mkRegex pat
          ismatch r inp = case matchRegex r inp of Nothing -> False
                                                   Just _  -> True
...
{- | Remove duplicate lines from a file (like Unix uniq). Takes a String
   representing a file or output and plugs it through lines and then nub
   to uniqify on a line basis.
 -}
uniq :: String -> String
